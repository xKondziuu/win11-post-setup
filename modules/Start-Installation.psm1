function Start-Installation {
  ...
  Write-Host "Starting installation `"$Path`" with arguments: $Arguments"
  ...
  $startInfo.UseShellExecute = -not $NoNewWindow
  $startInfo.CreateNoWindow = $true

  try {
    $process = [System.Diagnostics.Process]::Start($startInfo)
  } catch {
    Write-Warning "Failed to start installer `"$Path`", skipping."
    return $false
  }
